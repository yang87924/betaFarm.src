...
FERT_PURCH_RECORD = Blueprint('FERT_PURCH_RECORD', __name__)

@FERT_PURCH_RECORD.route('/api/FERT_PURCH_RECORD', methods=['POST', 'PUT'])
@required_login
@trace_sys_flag_check
def api_FERT_PURCH_RECORD():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            g.session.add(db.models.FERT_PURCH_RECORD(
                farm_id=farm_id,
                region_id=region_id,
        ...
        if farm_active:
            for id in farm_active:
                farm_id += str(id[0])
        record = (g.session
                   .query(db.models.FERT_PURCH_RECORD)
                   .filter(db.models.FERT_PURCH_RECORD.record_id == record_id)
        ...
        memo = request.form.get('memo')
        up_file = request.files['up_file']
        sup_id = sup_id = g.session.query(db.models.ITEMS).filter(
            db.models.ITEMS.item_no == material_code).first().sup_id
        lot_number = request.form.get('lot_number')
        ...
        max_old_counter = (g.session
                            .query(db.models.FERT_PURCH_RECORD)
                            .filter(db.models.FERT_PURCH_RECORD.record_id == record_id)
                            .order_by(db.models.FERT_PURCH_RECORD.is_old.desc())
                            .first())
