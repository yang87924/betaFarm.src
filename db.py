...
import config
import models
import tiaga_modules

engine = None

def tiaga_connect(mac):
    mac_engine = create_engine(config.TIAGA_MODULE_MAC_LIST[mac],
                                pool_recycle=config.DB_POOL_RECYCLE)

    Session = sessionmaker(bind=mac_engine)
    session_tiaga = Session()
    return session_tiaga
